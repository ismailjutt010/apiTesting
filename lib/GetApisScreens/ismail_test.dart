...
class IsmailTest extends StatefulWidget {
  const IsmailTest({super.key});

  @override
  State<IsmailTest> createState() => _IsmailTestState();
}

class _IsmailTestState extends State<IsmailTest> {
 List<ProductsModel> products = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Center(child: Text('Ismail Test'))),
      body: Padding(
                  ...
                  itemCount:snapshot.data.data.length , itemBuilder:(context , index) =>
                    Column(
                      ...
                      children: [
                        ...
                        Icon(snapshot.data.data[index].inWishlist.toString() == false ? Icons.favorite : Icons.favorite_outline)
                      ],
                    ), );
              }else{
                return Text('Loading');
